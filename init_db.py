...
class EntrepriseDatabase:
    def __init__(self, db_path=globals.DB_PATH):
        ...
        if not os.path.exists(globals.DATA_DIR):
            print("Creating data directory at:", globals.DATA_DIR)
        ...
        if self.get_all_entreprises():
            ...
            df.drop(
                [
                    ...
                    "Tél. fixe",
                    "Solde global",
                    "ColonneVisibiliteClientInfobulle",
                    "Tél. portable",
                    "Rég. fisc.",
                    "Cat. rev.",
                    "Colla. resp.",
            ...
            pattern = (
                r"\b(?:"
                + "|".join(re.escape(forme) for forme in formes_juridiques)
                + r")\b\.?"
            )

            df["raison_sociale"] = (
                df["raison_sociale"].str.replace(pattern, "", regex=True).str.strip()
            )
